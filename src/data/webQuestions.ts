...
{ "id": 42, "name": "Sarah Connor", "role": "Admin" }`,
    ...
    followUps: [
      ...
      },
      {
        ...
        answer: 'A mechanism that allows multiple HTTP requests and responses to be sent over a single TCP connection, avoiding the overhead of repeating TCP and TLS handshakes.'
      }
    ],
    keyPointsToMention: [
...
this.http.post<User>('/api/users', { name: 'Alex', department: 'Engineering' });`,
    ...
    followUps: [
      ...
      {
        question: 'What happens if a browser automatically retries a failed POST request?',
        answer: 'If the server processed the request before the network dropped, retrying POST can cause duplicate database inserts or double payments, which is why POST requires idempotency handling.'
      }
    ],
    keyPointsToMention: [
    ...
    followUps: [
      {
        ...
        answer: 'Strictly speaking, PATCH is not guaranteed to be idempotent by the HTTP spec (e.g. an append patch or increment operation), whereas PUT is always idempotent.'
      },
      {
        question: 'What is JSON Patch (RFC 6902)?',
        answer: 'A standard format for describing a sequence of operations to apply to a JSON document (e.g. `[ { "op": "replace", "path": "/email", "value": "new@work.com" } ]`).'
      }
    ],
    keyPointsToMention: [
...
PATCH    | No                | Not necessarily (e.g. increment operation)`,
    ...
    followUps: [
      ...
      {
        question: 'How do you make non-idempotent POST endpoints idempotent in distributed systems?',
        answer: 'Use Idempotency Keys generated by the client and tracked in Redis or database unique constraint tables before processing.'
      }
    ],
    keyPointsToMention: [
...
- HTTP/3: Runs over QUIC protocol (UDP), 0-RTT connection resumption, stream-independent packet recovery.`,
    ...
    followUps: [
      ...
      {
        question: 'What is Server Name Indication (SNI)?',
        answer: 'An extension to TLS where the client indicates the hostname it is trying to connect to at the start of the TLS handshake, allowing a single IP address to host multiple secure domains with different SSL certificates.'
      }
    ],
    keyPointsToMention: [
...
\`;`,
    ...
    followUps: [
      ...
      {
        question: 'What is the N+1 problem in GraphQL resolvers and how is it solved?',
        answer: 'When resolving nested relationships, a naive resolver runs 1 query for parents and N queries for children. It is solved using DataLoader to batch and cache sub-queries.'
      }
    ],
    keyPointsToMention: [
...
FETCH NEXT 20 ROWS ONLY;`,
    ...
    followUps: [
      ...
      {
        question: 'When is Offset pagination still the better choice?',
        answer: 'For administrative tables and back-office reports where datasets are small (< 5,000 rows) and users explicitly need to jump to specific page numbers or see total page counts.'
      }
    ],
    keyPointsToMention: [
